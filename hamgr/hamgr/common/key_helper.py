...
# /etc/pf9/global.conf , used when settings not in hamgr.conf
global_config = os.path.join(global_config_base, 'global.conf')
...
def get_general_configs():
    ...
    # if no required settings from hamgr config, fall back to
    # global for second chance
    if not configs['du_fqdn'] or \
            not configs['customer_shortname'] or \
            not configs['customer_fullname'] or \
            not configs['region_name']:
        LOG.debug('du general settings not exist in %s, now try %s',
                  hamgr_config, global_config)
