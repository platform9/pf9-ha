...
class NovaProvider(Provider):
    ...
    def process_host_aggregate_changes(self):
        ...
        for cluster in clusters:
            ...
            try:
                ...
                if cluster.task_state in [constants.TASK_COMPLETED]:
                ...
                if inactive_host_ids or \
                        cluster.task_state not in [constants.TASK_COMPLETED]:
    ...
    def _customize_pf9_ha_slave_config(self, consul_join_ips, consul_host_ip, consul_cluster_ip):
        ...
        customize_cfg['role_rebalance_enabled'] = is_enabled
    ...
    def _handle_disable_request(self, request, next_state=constants.TASK_COMPLETED):
        ...
        try:
            ...
            LOG.debug('delete failover segment from masakari')
    ...
    def _report_event(self, event_type, event_details):
        ...
        try:
            ...
            before_rebalance = event_details['consul']
            ...
            msg = 'found %s alive consul servers : %s' % (str(len(consul_servers_alive)), str(consul_servers_alive))
            LOG.info(msg)
            msg = 'found %s alive consul slaves : %s' % (str(len(consul_slaves_alive)), str(consul_slaves_alive))
            LOG.info(msg)
            ...
            if len(consul_servers_alive) > server_threshold:
                ...
                rebalance_type = 'server role to slave role'
                ...
                for i in range(start, end):
                    candicates.append(consul_servers_alive[i - start])
                LOG.info('more than %s alive consul servers (found %s), rebalance those to consul slaves : %s',
                         str(server_threshold),
                         str(len(consul_servers_alive)),
                         str(candicates))
            elif len(consul_servers_alive) < server_threshold:
                rebalance_type = 'slave role to server role'
                wanted = server_threshold - len(consul_servers_alive)
                LOG.info('not enough alive consul servers, found %s, required %s, wanted %s', str(len(consul_servers_alive)),
                         str(server_threshold),
                         str(wanted))
                if wanted == server_threshold:
                    LOG.error('ignore rebalance as there are no alive consul slaves to rebalance')
                else:
                    ...
                    for i in range(start, end):
                        candicates.append(consul_slaves_alive[i - start])
                    LOG.info('less than %s alive consul servers (found %s), rebalance those to consul servers',
                             str(server_threshold),
                             str(len(consul_servers_alive)),
                             str(candicates))
            else:
                LOG.info('no need to rebalance consul roles, num of alive consul servers %s meets required %s',
                         str(len(consul_servers_alive)),
                         str(server_threshold))

            if len(candicates) > 0:
                LOG.info('consul role rebalance needed (%s) for those hosts : %s', rebalance_type, str(candicates))
            # create rebalance request for each target hosts, they should be processed one by one
            for candidate in candicates:
                host = candidate['Name']
                rebalance_action = {'host': host, 'old_role': old_role, 'new_role': new_role}
                LOG.info('create consul role rebalance request for host %s : %s', host, str(rebalance_action))
                db_api.add_consul_role_rebalance_record(event_type,
                                                        event_uuid,
                                                        json.dumps(before_rebalance),
    ...
    def process_consul_role_rebalance_requests(self):
        ...
        LOG.info('role rebalance processing task start to work at %s', str(datetime.utcnow()))
        try:
            # ----------------------------------------------------------------
            ...
            for req in unhandled_requests:
                LOG.info('processing rebalance request : %s', str(req))
                # abort request if timestamp shows the request older than 15 minutes
                if (datetime.utcnow() - req.last_updated) > timedelta(minutes=15):
                    db_api.update_consul_role_rebalance(req.uuid,
                                                        ...
                                                        'ignore staled request created at %s' % req.last_updated)
                    continue
                ...
                if not host_event:
                    ...
                    db_api.update_consul_role_rebalance(req.uuid,
                                                        ...
                                                        None,
                                                        constants.REBALANCE_STATE_ABORTED,
                                                        'no such host event %s for rebalance' % event_uuid)
                    continue

                # check whether host event has finished
                event_status = host_event.notification_status
                if event_status != constants.STATE_FINISHED:
                    LOG.warning('host event %s has not finished, current status: %s', event_uuid, event_status)
                ...
                if event_status == constants.STATE_FINISHED:
                    ...
                    result = requests.get(url + "/" + target_host_id, headers=headers)
                    data = result.json()
                    assert result.status_code == requests.codes.ok and 'pf9-ha-slave' in data['roles']
                    data = {}
                    consul_status_before = json.loads(req.before_rebalance)
                    LOG.info('getting new consul settings from previous status : %s', str(req.before_rebalance))
                    ...
                    LOG.info('update resmgr for host %s with data %s', target_host_id, str(data))
                    ...
                    if not succeeced:
                        LOG.warn('failed to update resmgr for host %s with data %s, resp %s',
                                 target_host_id,
                                 str(data),
                                 str(result))
                        # update the status to aborted if unable to update resmgr
                        db_api.update_consul_role_rebalance(req.uuid,
                                                            None,
                                                            None,
                                                            constants.REBALANCE_STATE_ABORTED,
                                                            'update resmgr failed for host %s ' % target_host_id)
                    else:
                        ...
                        LOG.info('role rebalance response : %s', str(resp))
                        if resp:
                            if resp['status'] == constants.REBALANCE_STATE_FINISHED:
                                LOG.info('role rebalance has successfully completed, resp %s', str(resp))
                                db_api.update_consul_role_rebalance(req.uuid,
                                                                    None,
                                                                    None,
                                                                    constants.REBALANCE_STATE_FINISHED,
                                                                    None)
                            else:
                                msg = 'rebalance response status is not finished , but %s' % str(resp)
                                LOG.info(msg)
                                db_api.update_consul_role_rebalance(req.uuid,
                                                                    None,
                                                                    None,
                                                                    constants.REBALABCE_STATE_ERROR,
                                                                    msg)
                        else:
                            msg = 'empty rebalance response for request : %s' % str(rebalance_request)
                            LOG.info(msg)
                            db_api.update_consul_role_rebalance(req.uuid,
                                                                None,
                                                                None,
                    ...
                    LOG.info('host event %s has unexpected status, ignore consul role rebase request',
                             str(host_event))
                    db_api.update_consul_role_rebalance(req.uuid,
                                                        None,
                                                        None,
                                                        constants.REBALANCE_STATE_ABORTED,
                                                        "unexpected status for host event %s : %s" % (event_uuid,
                                                                                                      event_status))
        except:
            LOG.exception('unhandled exception in process_consul_role_rebalance_requests')
        finally:
            # release thread lock
            with self.consul_role_rebalance_processing_lock:
                self.consul_role_rebalance_processing_running = False
        LOG.info('role rebalance processing task has finished at %s', str(datetime.utcnow()))
