...
class ResmgrClient:
    ...
    def get_app_info(self, host_id, token, bbmaster_host):
        url = "http://%s:8082/v1/hosts/%s/apps" % (bbmaster_host, host_id)
        return self._send_request("get", url, token)
    ...
    def fetch_app_details(self, host_ids, token, bbmaster_host):
        app_details = {}
        for host_id in host_ids:
            app_info = self.get_app_info(host_id, token, bbmaster_host)
